...
export default function AgentLogPanel({
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--border)] flex-shrink-0" style={{ background: "var(--surface-2)" }}>
        <div className="flex items-center gap-2">
          <span className="font-medium text-sm text-[var(--text)]">Agent Log</span>
          {jobCount > 0 && (
            <span className="text-[11px] liquid-glass rounded-lg px-2 py-0.5 text-[var(--text-muted)]" style={{ background: "var(--surface)" }}>
              {jobCount} jobs
            </span>
          )}
        </div>
        <button onClick={onClear} className="text-xs text-[var(--text-muted)] hover:text-[var(--text)] transition-colors">Clear</button>
      </div>

      <div className="flex-1 overflow-y-auto p-3 font-mono text-[12px] leading-relaxed" style={{ minHeight: 0, background: "var(--bg)", backdropFilter: "blur(8px)" }}>
      ...
      {sourcesTotal > 0 && (
        <div className="px-4 py-3 border-t border-[var(--border)] flex-shrink-0" style={{ background: "var(--surface-2)" }}>
          <div className="w-full rounded-full h-1 mb-2" style={{ background: "var(--border)" }}>
