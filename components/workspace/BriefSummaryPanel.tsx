...
const PlatformIcon = ({ platform }: { platform: string }) => {
  if (platform === "youtube") return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-[#C0392B] flex-shrink-0">
...
export default function BriefSummaryPanel({ campaign, sources, processedSourceIds, onRescan }: Props) {
  ...
  return (
    <div className="h-full overflow-y-auto bg-gray-50 p-3 flex flex-col gap-3 text-sm">
      {/* Brief Summary */}
      <div>
        <p className="text-[11px] font-bold text-[#6B7280] uppercase mb-2">Brief Summary</p>
      ...
      {(contentRules.length > 0 || rejectionReasons.length > 0) && (
        <div>
          <p className="text-[11px] font-bold text-[#6B7280] uppercase mb-2">Content Rules</p>
      ...
      <div>
        <p className="text-[11px] font-bold text-[#6B7280] uppercase mb-2">
          Source Videos
          <span className="ml-1 font-normal normal-case text-[#6B7280]">({sources.length} available)</span>
        </p>
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="max-h-48 overflow-y-auto divide-y divide-border/50">
            {sources.map((src) => {
              ...
              return (
                <div key={src.id} className="flex items-center gap-2 px-3 py-2">
                  ...
                  <span className="flex-1 text-xs text-[#111827] truncate">{label}</span>
                  {done ? (
            ...
            {sources.length === 0 && (
              <p className="text-xs text-[#6B7280] text-center py-4">No sources yet</p>
            )}
          </div>
          <div className="flex gap-2 p-2 border-t border-gray-200">
            <button className="flex-1 flex items-center justify-center gap-1 text-xs border border-gray-200 rounded-lg py-1.5 text-[#6B7280] hover:bg-gray-50">
              <Plus size={11} /> Add Sources
            </button>
            <button
              onClick={onRescan}
              className="flex-1 flex items-center justify-center gap-1 text-xs border border-gray-200 rounded-lg py-1.5 text-[#6B7280] hover:bg-gray-50"
            >
      ...
      {/* Schedule */}
      <div>
        <p className="text-[11px] font-bold text-[#6B7280] uppercase mb-2">Schedule</p>
        <div className="bg-white rounded-xl border border-gray-200 p-3">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-[#111827]">Auto-run</span>
            <button
              onClick={() => setAutoRun(!autoRun)}
              className={`w-9 h-5 rounded-full transition-colors relative ${autoRun ? "bg-[#C0392B]" : "bg-gray-200"}`}
            >
              <div className={`w-3.5 h-3.5 bg-white rounded-full absolute top-0.5 shadow transition-all ${autoRun ? "left-[19px]" : "left-0.5"}`} />
            </button>
          </div>
          {autoRun && (
            <div>
              <select className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1.5 mb-1.5" onClick={() => setShowSchedule(true)}>
                <option>Daily</option>
                <option>Every 12 hours</option>
                <option>Every 6 hours</option>
                <option>Weekly</option>
              </select>
              <p className="text-[11px] text-[#6B7280]">Next run: in 24 hours</p>
            </div>
          )}
          {showSchedule && null}
        </div>
