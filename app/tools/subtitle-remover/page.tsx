    ...
    <ToolPageLayout title="Subtitle Remover">
      <div className="space-y-4">
        ...
        >
          ...
          <p className="text-sm font-semibold text-[#111827]">{file ? file.name : "Drop video here or click to upload"}</p>
          <p className="text-xs text-[#6B7280] mt-1">MP4, MOV, AVI supported</p>
          <input ref={inputRef} type="file" accept="video/*" className="hidden" onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} />
        </div>

        {videoPreview && (
          <video src={videoPreview} controls className="w-full rounded-xl border border-gray-200 max-h-48 object-contain bg-black" />
        )}
