...
const STEPS = [
  {
    step: "01",
    icon: <Zap size={20} className="text-[#9B1C1C]" />,
    title: "Run the Agent",
  ...
  {
    step: "02",
    icon: <CheckCircle size={20} className="text-[#9B1C1C]" />,
  ...
  {
    step: "03",
    icon: <Upload size={20} className="text-[#9B1C1C]" />,
    title: "Post the Clip",
  ...
  {
    step: "04",
    icon: <DollarSign size={20} className="text-[#9B1C1C]" />,
    title: "Get Paid",
...
const REWARDS: { platform: string; program: string; rate: string; notes: string; color: string }[] = [
  ...
  { platform: "Campaign CPM", program: "Brand Campaign Payouts", rate: "Set by each campaign", notes: "Paid by the brand directly. Check each campaign card for CPM, Max/clip, and Min payout", color: "#9B1C1C" },
];
          ...
          {STEPS.map((s) => (
            <div key={s.step} className="flex gap-4">
              <div className="flex-shrink-0 flex flex-col items-center">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "#FFF5F5", border: "1px solid #FECACA" }}>
                  {s.icon}
                </div>
              ...
              <div className="pb-6 flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] font-bold text-[#9B1C1C] tracking-widest">STEP {s.step}</span>
                </div>
                ...
                <ul className="space-y-1.5">
                  {s.tips.map((t, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#0F1E3C]">
                      <span className="mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: "#9B1C1C" }} />
                      {t}
                    </li>
                  ))}
                </ul>
            ...
            <div key={r.platform} className="flex gap-4 p-4 rounded-xl" style={{ background: "#F8FAFC", border: "1px solid #E2E8F0" }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  ...
                  <span className="text-xs font-bold px-2.5 py-0.5 rounded-full flex-shrink-0" style={{ background: "#FFF5F5", color: "#9B1C1C", border: "1px solid #FECACA" }}>{r.rate}</span>
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {TIPS.map((t, i) => (
            <div key={i} className="p-4 rounded-xl" style={{ background: "#F8FAFC", border: "1px solid #E2E8F0" }}>
              <div className="flex items-center gap-2 mb-2 text-[#9B1C1C]">
                {t.icon}
      ...
      <AccordionSection title="✅ Campaign Requirements Checklist">
        ...
        <div className="space-y-2">
          {[
            ...
            <label key={i} className="flex items-start gap-3 p-3 rounded-xl cursor-pointer hover:bg-[#FFF5F5] transition-colors group">
              <input type="checkbox" className="mt-0.5 accent-[#9B1C1C] w-4 h-4 flex-shrink-0" />
