...
function ConnectedAccounts() {
  ...
  return (
    <div id="social" className="panel p-6">
      <div className="mb-4">
        <h2 className="font-semibold text-[var(--text)]">Connected Accounts</h2>
        <p className="text-xs text-[var(--text-muted)] mt-0.5">Connect your social accounts to post clips directly from Montview.</p>
      </div>
      <div className="h-px bg-[var(--border)] -mx-6 mb-4" />
      <div className="space-y-3">
        {platforms.map(p => {
          const acct = accounts.find(a => a.platform === p.key);
          return (
            <div key={p.key} className="flex items-center gap-4 p-4 rounded-xl" style={{ background: "#F8FAFC", border: "1px solid var(--border)" }}>
              <span className="text-2xl">{p.icon}</span>
              <div className="flex-1">
                <p className="font-semibold text-sm text-[var(--text)]">{p.label}</p>
                {acct
                  ? <p className="text-xs text-green-600 mt-0.5">✓ Connected as <strong>{acct.accountName}</strong></p>
                  : <p className="text-xs text-[var(--text-light)] mt-0.5">Not connected</p>}
              </div>
              {acct ? (
...
function AppCredentials() {
  ...
  const field = (label: string, k: keyof typeof keys, placeholder: string) => (
    <div>
      <label className="text-xs font-bold text-[var(--text-muted)] block mb-1.5">{label}</label>
  ...
  return (
    <div className="panel p-6">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h2 className="font-semibold text-[var(--text)]">App Credentials</h2>
          <p className="text-xs text-[var(--text-muted)] mt-0.5">Paste your developer keys — required to connect TikTok, Instagram &amp; YouTube.</p>
        </div>
      ...
      <div className="h-px bg-[var(--border)] -mx-6 mb-5" />
...
function MaskedInput({ label, placeholder }: { label: string; placeholder: string }) {
  ...
  return (
    <div>
      <label className="text-xs font-semibold text-[var(--text-muted)] block mb-1.5">{label}</label>
      <div className="relative">
        <input
...
export default function SettingsPage() {
  ...
  if (!loaded) return (
    <div className="flex items-center justify-center h-64">
      <Loader2 size={24} className="animate-spin text-[var(--text-light)]" />
    </div>
  );
    ...
    <div className="max-w-2xl space-y-5 pb-10">
      <div className="flex items-center justify-between mb-2 animate-fade-up delay-1">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)] mt-0.5">Configure Montview to work with your accounts and preferences</p>
        </div>
          ...
          <div className="space-y-5">
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-semibold text-[var(--text-muted)]">Default Clip Length</label>
                <span className="text-sm font-bold text-[var(--text)]">{settings.defaultLength}s</span>
              </div>
            ...
            <div>
              <label className="text-xs font-semibold text-[var(--text-muted)] block mb-1.5">Default Aspect Ratio</label>
            ...
            <div>
              <label className="text-xs font-semibold text-[var(--text-muted)] block mb-1.5">Subtitle Style</label>
            ...
            <div>
              <label className="text-xs font-semibold text-[var(--text-muted)] block mb-1.5">Color Grade</label>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-semibold text-[var(--text-muted)]">Max Clips Per Video</label>
                <span className="text-sm font-bold text-[var(--text)]">{settings.maxClipsPerVideo}</span>
              </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-[var(--border)]">
              <div>
                <p className="text-sm font-semibold text-[var(--text)]">Auto-edit on generate</p>
                <p className="text-xs text-[var(--text-muted)] mt-0.5">Apply color grade, subtitles, hook text, and watermark automatically</p>
              </div>
      ...
      <div className="animate-fade-up delay-5">
        <Section title="Storage">
          <div>
            <label className="text-xs font-semibold text-[var(--text-muted)] block mb-1.5">Output Folder</label>
