...
// Public HTTPS URL registered with TikTok/Meta as the OAuth redirect.
// TikTok rejects localhost, so the callback is hosted on Vercel and forwards back to the local app.
...
export async function GET(
  ...
  if (platform === "tiktok") {
    ...
    const redirect = encodeURIComponent(`${OAUTH_REDIRECT}/api/social/callback/tiktok`);
  ...
  if (platform === "instagram") {
    ...
    const redirect = encodeURIComponent(`${OAUTH_REDIRECT}/api/social/callback/instagram`);
